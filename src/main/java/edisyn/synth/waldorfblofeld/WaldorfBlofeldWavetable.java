...
import edisyn.Synth;
import edisyn.gui.HBox;
import edisyn.gui.SelectedTextField;
import edisyn.gui.VBox;
import edisyn.gui.WidgetList;
import edisyn.util.StringUtility;
import edisyn.util.WavFile;
import edisyn.util.WavFileException;
import edisyn.util.WindowedSinc;
...
public class WaldorfBlofeldWavetable {
    ...
    /**
     * Select a file (WAV, sysex, or mid), and then load a wavetable from that
     * file. If it's a WAV file, call uploadWAV() -- this assumes it's a
     * WaveEdit file. If it's MID or SYX, call uploadSysex() -- this assumes
     * it's one or more Blofeld wavetables in standard sysex format.
     */
    ...
    /**
     * Extract the sysex and guess whether it's one wavetable or more. This is
     * fragile: we're just grabbing the valid sysex chunks and grouping them
     * into arrays of 64 and assuming each array is one wavetable. We're not
     * presently checking to see if the arrays are right. If there's only one
     * group, then we call uploadOneSysex() to upload it. Else we call
     * uploadMultiSysex() to pick a group.
     */
    void uploadSysex(WaldorfBlofeld synth, File file) throws IOException {
        ...
        List<byte[][]> sysexA = new ArrayList<>();
    ...
    /**
     * Ask the user to specify the device ID, wavetable number, and wavetable
     * name. Then we modify the sysex files appropriately and upload them.
     */
    ...
    /**
     * Ask the user to specify the device ID, wavetable number, and wavetable
     * name. Then we call writeData() to upload them.
     */
    ...
    /**
     * Load the data from the file and convert it into a Blofeld wavetable. This
     * is done by breaking the WAV file into chunks 256 or 2048 samples long
     * (the length of a WaveEdit or Serum wavetable wave), then resampling them
     * using Windowed Sinc Interpolation to chunks 128 samples long (the length
     * of a Blofeld wavetable wave), then building the wavetable sysex from
     * these resulting waves and uploading them.`
     */
...
true);
    ...
    public void reviseWavetableNames(Synth synth) {
        while (true) {
            ...
            List<List<JTextField>> textFields = new ArrayList<>(3);
            List<List<String>> labels = new ArrayList<>(3);
            for (int j = 0; j < 3; j++) {
                List<JTextField> tf = new ArrayList<>(13);
                List<String> l = new ArrayList<>(13);
                for (int k = 0; k < 13; k++) {
                    int i = j * 13 + k;
                    l.add("" + (i + 80));
                    ...
                    tf.add(new JTextField(name, BLOFELD_WAVETABLE_NAME_LENGTH));
                }
                textFields.add(tf);
                labels.add(l);
            }

            WidgetList list1 = new WidgetList(labels.get(0), textFields.get(0));
            WidgetList list2 = new WidgetList(labels.get(1), textFields.get(1));
            WidgetList list3 = new WidgetList(labels.get(2), textFields.get(2));
            ...
            if (ret == 0) // update
            {
                for (int j = 0; j < 3; j++) {
                    for (int k = 0; k < 13; k++) {
                        int i = j * 13 + k;
                        synth.setLastX(textFields.get(j).get(k).getText().trim(), "WTName" + i, synth.getSynthClassName(), true);
                    }
                }
                ((WaldorfBlofeld) synth).rebuildWavetables();
                return;
