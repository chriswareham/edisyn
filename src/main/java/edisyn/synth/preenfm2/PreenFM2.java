...
public class PreenFM2 extends Synth
        ...
        public int reviseMutatedValue(String key, int old, int current)
            ...
            if (mutationRestriction == OFF)
                ...
                if (mutationRestriction == TX81Z)
                    return findClosestValue(current, YAMAHA_FREQUENCY_RATIOS);
    ...
    /**
     * A useful utility function which returns the element in the sorted (low to
     * high) array A which is closest to VALUE. You could use this to search
     * arrays for alt values for rounding for example.
     */
    private static int findClosestValue(final int value, final int[] a) {
        if (value < a[0]) {
            return a[0];
        }
